...
\connect students SET statement_timeout = 0;
...
SET
    DEFAULT nextval(
        'public.courses_course_id_seq':: regclass
    );
...
SET
    DEFAULT nextval(
        'public.majors_major_id_seq':: regclass
    );
...
SET
    DEFAULT nextval(
        'public.students_student_id_seq':: regclass
    );
...
ALTER TABLE
    ONLY public.majors_courses
ADD
    CONSTRAINT majors_courses_pkey PRIMARY KEY (major_id, course_id);
...
ALTER TABLE
    ONLY public.majors_courses
ADD
    CONSTRAINT majors_courses_course_id_fkey FOREIGN KEY (course_id) REFERENCES public.courses(course_id);
...
ALTER TABLE
    ONLY public.majors_courses
ADD
    CONSTRAINT majors_courses_major_id_fkey FOREIGN KEY (major_id) REFERENCES public.majors(major_id);
